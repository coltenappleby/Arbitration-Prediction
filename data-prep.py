...
def get_fangraphs_data():
	...
	url = f"https://www.fangraphs.com/api/leaders/major-league/data?age=&pos={position}&stats={stats}&lg=all&qual={qual}&season={end_year}&season1={start_year}&startdate=&enddate=&month=0&hand=&team=0&pageitems=2000000000&pagenum=1&ind=0&rost=0&players=&type=8&postseason=&sortdir=default&sortstat=WAR"
	response = requests.get(url)
	response.raise_for_status()
	json_data = response.json()
	df = pd.DataFrame(json_data['data'])
	df.to_csv(f'./data/mlb-stats-{stats}-{start_year}-{end_year}.csv', index=False)
